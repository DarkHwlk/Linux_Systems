...
/*
 * Y TUONG BAI TOAN:
 * +/ producer: mat mot khoang thoi gian de san xuat do do an
 * ra mot bang chuyen, sao cho du 10 mon tren bang truyen thi 
 * se ngung, khong ra do nua
 * +/ costumer: Tieu thu do an trong mot khoang thoi gian nhat
 * dinh, khi het thuc an tren bag truyen thi customer se phai 
 * doi
 * */
...
int main()
{
	...
	/* Unnamed semaphore sem_init(sem, mode < 0: threads | 1: processes >, amountOfSem) */
    sem_init(mutex, 1, 1);
    ...
    else
    {
        while(1)
        {
            sleep(10);
            int takenSlots;
            sem_getvalue(full, &takenSlots);  // Get value of sem to save in takenSlots 
