...
int main()
{
     ...
     */
    key_t key = ftok("./shmfile", 65);  //Tao key, id cac process dung chung shared memory phai giong nhau
     ...
     */
    int shmid = shmget(key, 1024, 0666 | IPC_CREAT);  // Tao shared memory
     ...
     */
    char *shmaddr = (char*)shmat(shmid, (void*)0, 0);  // Atach vung nho cua process toi shared memory
  
    printf("Data read from memory: %s\n", shmaddr);  //In ra du lieu trong shmaddr da mappng toi shared memory
     ...
     * 
     */
    shmctl(shmid, IPC_RMID, NULL);  //Xoa shared memory
